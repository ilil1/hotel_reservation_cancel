...
호텔 예약이 실제 숙박 전에 취소되면 빈 객실과 매출 손실이 발생할 수 있습니다. 이 프로젝트는 예약 당시 확인할 수 있는 리드타임, 객실요금, 보증금 유형, 특별 요청 수, 예약 경로 등의 정보를 사용해 **해당 예약이 향후 취소될 가능성**을 예측합니다.
...
- 원본 예약 정보로부터 총 숙박일, 총 투숙객, 과거 취소율 등 9개 파생 특성 추가
- 익명 식별번호인 `agent`, `company`를 숫자 크기가 아닌 범주형으로 처리
...
EDA를 먼저 완료한 후 전처리 단계에서 Feature Engineering을 수행합니다. 정답인 `is_canceled`는 파생 특성 생성에 사용하지 않으므로 데이터 누수가 발생하지 않습니다.

| 파생 특성 | 생성 방법 | 의미 |
|---|---|---|
| `total_nights` | 평일 숙박일 + 주말 숙박일 | 전체 숙박 기간 |
| `total_guests` | 성인 + 어린이 + 영아 | 예약의 총 투숙 인원 |
| `is_family` | 어린이 또는 영아가 있으면 1 | 가족 단위 예약 여부 |
| `previous_bookings_total` | 과거 취소 + 과거 정상 예약 | 고객의 전체 과거 예약 수 |
| `previous_cancellation_rate` | 과거 취소 / 과거 전체 예약 | 고객의 과거 취소 비율 |
| `has_special_requests` | 특별 요청이 1개 이상이면 1 | 특별 요청 여부 |
| `has_booking_changes` | 예약 변경이 1회 이상이면 1 | 예약 변경 여부 |
| `is_agent_booking` | `agent`가 있으면 1 | 여행사·대행사 예약 여부 |
| `is_company_booking` | `company`가 있으면 1 | 기업 관련 예약 여부 |

`agent`와 `company`는 숫자로 저장되어 있지만 크기에 순서나 양적 의미가 없는 익명 식별번호입니다. 따라서 숫자형 스케일링 대상에서 제외하고 범주형으로 변환해 원-핫 인코딩합니다.

Feature Engineering 후 검증 F1이 가장 높은 Logistic Regression이 최종 모델로 선택되었습니다. 테스트 결과는 다음과 같이 변했습니다.

| 구분 | 선택 모델 | Accuracy | Precision | Recall | F1 |
|---|---|---:|---:|---:|---:|
| Feature Engineering 전 | Random Forest | 0.771 | 0.774 | 0.612 | 0.684 |
| Feature Engineering 후 | Logistic Regression | 0.737 | 0.628 | 0.859 | 0.726 |
...
기본 주소는 `http://localhost:8501`입니다. 왼쪽 메뉴에서 다음 세 화면을 선택할 수 있습니다.

- `모델 성능 대시보드`: EDA, 성능 요약, 모델 비교, 혼동행렬, Classification Report, Feature Importance 확인
- `신규 예약 1건 예측`: 예약 정보를 직접 입력해 취소 확률과 최종 판정 확인
- `CSV 일괄 예측 및 운영 활용`: CSV를 한 번 업로드한 뒤 예측 결과, 고객 확인·리마인더, 객실 재판매·수요관리를 세 개 탭에서 확인

Streamlit은 모델을 다시 학습하지 않습니다. 성능 화면은 `outputs/model/`의 평가 결과를 읽고, 예측 화면은 저장된 `model.joblib`을 불러와 `src/inference.py`의 공통 추론 파이프라인을 사용합니다. 데이터나 모델을 변경했다면 `train.py`를 먼저 다시 실행해야 합니다.

운영 활용 화면은 다음 정보를 제공합니다.

- 판단 임계값 이상인 고위험 예약과 고객 확인용 리마인더 문구를 CSV로 저장
- 예약별 취소 확률의 합계를 이용해 도착일별 예상 취소량과 예상 유지 예약 계산

원본 데이터에는 고객 연락처가 없으므로 메시지를 직접 발송하지는 않습니다. 실제 발송에는 고객 연락처가 있는 예약 시스템 또는 문자·이메일 서비스 연동이 필요합니다. 예상 취소량은 확정 취소가 아니므로 실제 취소 확인과 객실 재고를 함께 검토해야 합니다.
